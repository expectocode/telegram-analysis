...
def main():
    ...
    with open(filepath, 'r') as jsonfile:
        ...
        counters = [defaultdict(list) for keyword in keywords] #list of defaultdicts. one dd per keyword
        for event in events:
...
#    frequencies = [{key: l.count(True)/l.count(False) * 100 for key, l in counter.items()}] #had to be moved to loop
    ...
    plt.title("usage of {} in {}{}".format(keywords, filename, postfix))
...
#    plt.ylabel("Percentage of messages containing \"{}\"".format(keyword), size=14)
    ...
    if savefolder is not None: #if there is a given folder to save the figure in, save it there
        ...
        plt.savefig("{}/{}_in_{}.png".format(savefolder, keywords_string, filename))
